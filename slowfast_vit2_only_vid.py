...
class TransformerEncoder_cross(layers.Layer):
    def __init__(self, embed_dim, dense_dim, num_heads, **kwargs):
        super().__init__(**kwargs)
        self.embed_dim = embed_dim
        self.dense_dim = dense_dim
    ...
    def call(self, inputs_1, inputs_2, mask=None):
        if mask is not None:
            mask = mask[:, tf.newaxis, :]

        attention_output = self.attention(inputs_1, inputs_2, attention_mask=mask)
        inputs_mean = layers.Average()([inputs_1, inputs_2])
        proj_input = self.layernorm_1(inputs_mean + attention_output)
        proj_output = self.dense_proj(proj_input)
        return self.layernorm_2(proj_input + proj_output)
...
def get_compiled_model():
    ...
    inputs_1 = layers.Dense(256, activation=tf.nn.sigmoid)(inputs_1)
    x = PositionalEmbedding(
        sequence_length, embed_dim, name="frame_position_embedding")(inputs_1)
    x = TransformerEncoder(embed_dim, dense_dim, num_heads, name="transformer_layer")(x)
    x1 = tf.identity(x)
    x = layers.GlobalMaxPooling1D()(x)
    inputs_2 = layers.Dense(256, activation=tf.nn.sigmoid)(inputs_2)
    x2 = PositionalEmbedding(
        sequence_length, embed_dim, name="frame_position_embedding_2")(inputs_2)
    x2 = TransformerEncoder(embed_dim, dense_dim, num_heads, name="transformer_layer_1")(x2)
    x3 = TransformerEncoder_cross(embed_dim, dense_dim, num_heads, name="transformer_layer_2")(x1, x2)
    x3 = layers.GlobalMaxPooling1D()(x3)
    x3 = layers.Dense(256, activation='relu')(x3)
    inputs_3 = layers.Dense(512, activation = 'sigmoid')(inputs_3)
    inputs_3 = layers.Dropout(MY_DROPOUT)(inputs_3)
    inputs_3 = layers.Dense(128, activation = 'relu')(inputs_3)
    full_x = layers.Concatenate()([x, x3, inputs_3])
    full_x = layers.Dense(256, activation='relu')(full_x)
...
def run_experiment():
    filepath = f"../source/slowfast_temp_vit2_only_vid_checkpoint_{MY_DENSE_DIM}_{MY_NUM_HEADS}_{MY_DROPOUT}_{today_str}"
    ...
    #save the model
    model.save(f'../results/models/slowfast_temp_vit2_only_vid_{MY_DENSE_DIM}_{MY_NUM_HEADS}_{MY_DROPOUT}_{today_str}.h5')
...
preds_df.to_csv(f'../results/preds/preds_slowfast_temp_vit2_only_vid_{MY_DENSE_DIM}_{MY_NUM_HEADS}_{MY_DROPOUT}_{today_str}.csv')
true_df.to_csv(f'../results/preds/true_slowfast_temp_vit2_only_vid_{MY_DENSE_DIM}_{MY_NUM_HEADS}_{MY_DROPOUT}_{today_str}.csv')
...
preds_df.to_csv(f'../results/preds/val_preds_slowfast_temp_vit2_only_vid_{MY_DENSE_DIM}_{MY_NUM_HEADS}_{MY_DROPOUT}_{today_str}.csv')
true_df.to_csv(f'../results/preds/val_true_slowfast_temp_vit2_only_vid_{MY_DENSE_DIM}_{MY_NUM_HEADS}_{MY_DROPOUT}_{today_str}.csv')